...
public class Main {
	public static void main(String[] args) {
		...
		try {
			...
			String sql2 = "insert into g_artists (anem, a_type, a_year, debut, regdate)" + " values ('방탄소년단, '남성', '2010년대', '2013년', datetime('now', 'localtime'));";
			int cnt = stat2.executeUpdate(sql2);
			if (cnt>0)
				System.out.println("새로운 데이터가 추가되었습니다!");
			else
				System.out.println("[Error] 데이터추가 오류!");
			stat2.close();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
